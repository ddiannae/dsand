...
g.tree <- graph_from_literal(1-+2,1-+3,1-+4,2-+5,2-+6,
                             2-+7,3-+8,3-+9,4-+10)
par(mfrow=c(1, 3))
## NULL para que agarre la etiqueta del atributo "name"
## de la gráfica y NA para que no haya etiquetas
igraph_options(vertex.size=30, edge.arrow.size=0.5,
               vertex.label=NULL, edge.label=NULL)
## En vértices es el atributo name pero en aristas el 
## atributo se llama label
E(g.tree)$label = 1:9
...
## El layout bipartite solo pone los vértices de un tipo arriba y 
## los de otro, abajo
...
par(mfrow=c(1,1))
karate_nolabel <- delete_vertex_attr(karate, "label")
## En vértices, label tiene prioridad a name de etiqueta
plot(karate_nolabel, layout=l, 
     vertex.color=NA)
## En aristas no pela name, solo label
E(karate_nolabel)$label <- 1:ecount(karate_nolabel)
...
## strenght suma los pesos de las aristas incidetes a cada
## nodo
E(karate)$weight[15] <- 45
...
partidos <- levels(party.nums.f)
partidos[6] <- "Partido de Francia"
partidos <- c("E", "A",  "H", "C", "D",  "F", "G", "B",  "I")
levels(party.nums.f) <- partidos
relevel(party.nums.f, c("A", "B"))
V(fblog)$PolParty <- sub(" PCF - LCR", "Partido de Francia", V(fblog)$PolParty)
...
data(karate)
k.nbhds <- make_ego_graph(karate, order=2)
...
plot(k.1, vertex.label=NA,
     vertex.color=c("red", rep("lightblue", 25)))
plot(k.34, vertex.label=NA,
     vertex.color=c(rep("lightblue", 23), "red"))
neighbors(karate, V(karate)[2])
